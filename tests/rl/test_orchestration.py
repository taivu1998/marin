...
def test_run_rl_coordinator_uses_gpu_rollout_environment(monkeypatch):
    client = _FakeClient()
    hosted_runtime = _HostedRuntime(runtime=SimpleNamespace(), hosted_actors=[])
    config = SimpleNamespace(
        run_id="rl-test",
    ...
    assert len(client.submissions) == 2
    assert client.submissions[0].resources.device.kind == "tpu"
    assert client.submissions[1].resources.device.kind == "gpu"
    assert client.submissions[0].environment.extras == ["math", "tpu"]
    assert client.submissions[1].environment.extras == ["gpu", "math"]
    assert client.submissions[1].environment.pip_packages == ["vllm==0.13.0"]


def test_run_rl_coordinator_preserves_non_preemptible_worker_requests(monkeypatch):
    client = _FakeClient()
    hosted_runtime = _HostedRuntime(runtime=SimpleNamespace(), hosted_actors=[])
    config = SimpleNamespace(
        run_id="rl-test",
        ...
        pip_dependency_groups=["math"],
        run_config=_run_config(
            train_resources=ResourceConfig.with_tpu("v5p-8", regions=["us-central1"], preemptible=False),
            rollout_resources=ResourceConfig.with_tpu("v5p-8", regions=["us-central1"], preemptible=False),
        ),
    )
    ...
    assert len(client.submissions) == 2
    assert client.submissions[0].resources.preemptible is False
    assert client.submissions[1].resources.preemptible is False
...
def test_run_rl_coordinator_rejects_gpu_inflight_vllm(monkeypatch):
    client = _FakeClient()
    config = SimpleNamespace(
        run_id="rl-test",
        resolved_instance_id="rl-test",
